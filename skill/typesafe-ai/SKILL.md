...
<!-- super-browser-managed -->
## Super Browser installation

For this workspace, use the authenticated hosted Super Browser connection for live
Jev calls. Read [Super Browser integration](references/super-browser.md) for the
advisory API and shared $5 monthly budget. Read the
[complete offline documentation index](references/docs/INDEX.md) for API, SDK and
cookbook references. Each page records its official source and retrieval date.
The original upstream skill is preserved in references/docs/upstream/SKILL.md.
Weekly updates are staged for review before distribution.
If hosted MCP tools are not loaded, run `python3 scripts/evaluate.py --status`
from this skill directory. To evaluate, pipe the documented JSON request into
`python3 scripts/evaluate.py`. This helper reuses the existing Super Browser
connection from `~/.super-browser.env` or its environment variables. It never
stores or sends a local TypeSafe key. Reuse request_id after any timeout.
<!-- /super-browser-managed -->
